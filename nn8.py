"""
implement a linear regression neural network
...
lines = birth_file.readlines()
birth_file.close()

# get header
birth_header = lines[0].strip('\r\n').split('\t')

# get data
birth_data = [[float(x) for x in y.strip('\n').split('\t') if len(x) >= 1] for y in lines[1:] if len(y) >= 1]
